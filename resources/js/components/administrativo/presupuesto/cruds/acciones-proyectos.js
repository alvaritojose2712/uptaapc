...
const propsController = (igual) => {
  return {
    ...
    fecha: igual.fecha,
    especificas: igual.especificas,
  }
}
const searchKeys = ["nombre"]
...
export default class App extends Component{
  ...
  changeItems(event){
    ...
    let type = e.attributes["data-type"].value

    let especificas = [...this.state.especificas]
    ...
    switch (type){
      case "add":
        ...
        if (
          ...
          desEspecificaAdd&&
          fechaEspecificaAdd&&
          !especificas.filter(e=>e.nombre===nombreEspecificaAdd).length
        ) {
            especificas = especificas.concat({
              id: null,
              nombre: nombreEspecificaAdd,
              descripcion: desEspecificaAdd,
              fecha: fechaEspecificaAdd,
            })
        }
      break;
      
      case "remove":
        let index = e.attributes["data-id"].value
        especificas = especificas.filter((e,i)=>i!=index);
      break;
    }
    this.setState({
                  ...
                  <table className="table">
                    <tbody>
                      ...
                      <tr>
                        <td>Específicas</td>
                        <th>
                          {dataShow[iditemselect].especificas?
                            <ul className="list-group-item">
                              {dataShow[iditemselect].especificas.map(e=>
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td className="text-right">
                              <span className="font-weight-bold mb-2 d-block">Específicas</span>
                            </td>
                            <td className="">
                              <div className="input-group mb-3">
                                <input type="text" className="form-control" onChange={this.onChange} value={nombreEspecificaAdd} name="nombreEspecificaAdd" placeholder="Nombre de la específica..."/>
                                <input type="text" className="form-control" onChange={this.onChange} value={desEspecificaAdd} name="desEspecificaAdd" placeholder="Descripción..."/>
                                <input type="date" className="form-control" onChange={this.onChange} value={fechaEspecificaAdd} name="fechaEspecificaAdd"/>
                              
                             
                                <div className="input-group-prepend">
                                  <button onClick={this.changeItems} data-type="add" type="button" className="btn btn-success"><i className="fa fa-plus"></i></button>
                                </div>
                              </div>
                              ...
                              <ul className="list-group">
                                {
                                  especificas.map((e,i)=>
                                  <li className="list-group pointer" onClick={this.changeItems} data-id={i} data-type="remove" key={i}>
                                    <strong className="text-muted">{e.id?e.id:"(*Agregar)"} / {e.nombre}</strong>
                                  </li>
                                  )
                                }
                              </ul>
